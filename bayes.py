...
def spamTest():
    ...
    trainingSet = list(range(50));testSet=[]#need to convert range type into list, otherwise, will report TypeError: 
                                           # 'range' object doesn't support item deletion for del
    for i in range(10):
...
def localWords(feed1,feed0):
    ...
    for i in range(minLen):
        wordList = textPasrse(feed1['entries'][i]['summary'])
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(0)
    vocabList = createVocabList(docList)
    ...
    for docIndex in trainingSet:
        trainingSet.append(bagOfWords2VecMN(vocabList,docList[docIndex]))
